# AOTP11 + AWS S3 Example

This example uses @AOTP11/companion with a custom AWS S3 configuration.
...
`@AOTP11/companion` in a `.env` file. You may find useful to first copy the
`.env.example` file:
...
corepack yarn workspace @AOTP11-example/aws-companion start
```
