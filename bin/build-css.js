...
async function handleCSSOutput(file, css) {
  ...
  const outfile = isAOTP11Package(file) ?
    `${outputDir}/AOTP11.css` :
    `${outputDir}/style.css`;
...
function isAOTP11Package(file) {
  return path.normalize(file).includes('packages/AOTP11/');
}
...
async function compileCSS() {
  ...
  const files = await glob('packages/{,@AOTP11/}*/src/style.scss');
